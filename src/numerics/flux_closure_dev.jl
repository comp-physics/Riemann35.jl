...
@inline function flux_closure35_dev(
        M000,M100,M200,M300,M400,M010,M110,M210,M310,M020,M120,M220,M030,M130,M040,
        M001,M101,M201,M301,M002,M102,M202,M003,M103,M004,M011,M111,M211,M021,M121,
        M031,M012,M112,M013,M022, full::Bool=false)
    ...
    # S32-family closure: App.B (default, full=false) vs F26 eq.(40) full closure
    # (full=true; adds the S030/S300 cross-coupling + S110/S310/S130 terms). The two
    # coincide when S030=S300=0. `full` is a runtime kernel arg threaded from
    # run_gpu_3d(full_s32=…); false constant-folds to byte-identical App.B. Fox 2026-07-08.
    S320 = full ? ((S030*S300^2)/2 - S030 + S030*(S300^2 - S400 + 1))*S110 + (S400 - (3*S300^2)/2)*S120 + (-(3*S030*S300)/2)*S210 + ((3*S300)/2)*S220 + S030*S310 - S300/2 :
                  (S400 - (3*S300^2)/2)*S120 + ((3*S300)/2)*S220 - S300/2
    S230 = full ? ((S300*S030^2)/2 - S300 + S300*(S030^2 - S040 + 1))*S110 + (S040 - (3*S030^2)/2)*S210 + (-(3*S300*S030)/2)*S120 + ((3*S030)/2)*S220 + S300*S130 - S030/2 :
                  (S040 - (3*S030^2)/2)*S210 + ((3*S030)/2)*S220 - S030/2
    S302 = full ? ((S003*S300^2)/2 - S003 + S003*(S300^2 - S400 + 1))*S101 + (S400 - (3*S300^2)/2)*S102 + (-(3*S003*S300)/2)*S201 + ((3*S300)/2)*S202 + S003*S301 - S300/2 :
                  (S400 - (3*S300^2)/2)*S102 + ((3*S300)/2)*S202 - S300/2
    S203 = full ? ((S300*S003^2)/2 - S300 + S300*(S003^2 - S004 + 1))*S101 + (S004 - (3*S003^2)/2)*S201 + (-(3*S300*S003)/2)*S102 + ((3*S003)/2)*S202 + S300*S103 - S003/2 :
                  (S004 - (3*S003^2)/2)*S201 + ((3*S003)/2)*S202 - S003/2
    S032 = full ? ((S003*S030^2)/2 - S003 + S003*(S030^2 - S040 + 1))*S011 + (S040 - (3*S030^2)/2)*S012 + (-(3*S003*S030)/2)*S021 + ((3*S030)/2)*S022 + S003*S031 - S030/2 :
                  (S040 - (3*S030^2)/2)*S012 + ((3*S030)/2)*S022 - S030/2
    S023 = full ? ((S030*S003^2)/2 - S030 + S030*(S003^2 - S004 + 1))*S011 + (S004 - (3*S003^2)/2)*S021 + (-(3*S030*S003)/2)*S012 + ((3*S003)/2)*S022 + S030*S013 - S003/2 :
                  (S004 - (3*S003^2)/2)*S021 + ((3*S003)/2)*S022 - S003/2
