...
function correct_moments_hyperbolic_3D(M::AbstractVector)
    ...
    # Explicit indexing, not `M...`. Splatting a 35-element container lowers to
    # `_apply_iterate` and allocates 1136 B per call on the CPU (measured, and
    # reproduced from a minimal 35-argument function). It also does not lower on the
    # device at all -- see the note in body_force.jl. Bit-identical either way.
    return collect(correct_moments_dev(M[1],M[2],M[3],M[4],M[5],M[6],M[7],M[8],M[9],M[10],M[11],M[12],M[13],M[14],M[15],M[16],M[17],M[18],M[19],M[20],M[21],M[22],M[23],M[24],M[25],M[26],M[27],M[28],M[29],M[30],M[31],M[32],M[33],M[34],M[35]))
end
