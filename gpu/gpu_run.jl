...
function run_gpu_3d(M0::Array{Float64,4}, dx::Real, Ma::Real, nstep::Integer;
                    ...
                    comm=nothing, halo::Int=2, dts=nothing, order::Int=2, proj_first_order::Bool=false, riemann_solver::Symbol=:hll,
    ...
    while step < nstep
        ...
        used = multigpu ?
            march3d_slab_gpu!(Md, dx, Ma, k, comm; halo=halo, dts=seg,
                              vacuum_floor=vacuum_floor, order=order, proj_first_order=proj_first_order, riemann_solver=riemann_solver, threads=threads) :
            march3d_gpu!(Md, dx, Ma, k; dts=seg, vacuum_floor=vacuum_floor, order=order, proj_first_order=proj_first_order, riemann_solver=riemann_solver, threads=threads)
        t += sum(used); step += k
        dump!()
    end
