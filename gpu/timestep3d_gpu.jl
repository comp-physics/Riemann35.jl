...
# ---------------------------------------------------------------------------
function _bgk_kernel!(Mm, ncl::Int, dt::Float64, kn::Float64, pr::Float64, om::Float64, rk3::Bool)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    if idx <= ncl
        @inbounds begin
...
function march3d_gpu!(M_dev::CuArray{Float64,4}, dx::Real, Ma::Real, nstep::Integer;
                      ...
                      pressure_recon::Bool=false, stage_bgk::Bool=false, Kn::Real=Inf, Pr::Real=1.0, omega::Real=0.5, stage_bgk_exact::Bool=false, s3max::Real=4.0 + abs(Ma) / 2.0, threads::Int=128)
    ...
    bgk! = stage_bgk ?
        ((Xm, dtv) -> (@cuda threads=threads blocks=cld(nclM, threads) _bgk_kernel!(Xm, nclM, Float64(dtv), knf, prf, omf, sbe); nothing)) :
        ((Xm, dtv) -> nothing)
...
function march3d_slab_gpu!(M::CuArray{Float64,4}, dx::Real, Ma::Real, nstep::Integer, comm;
                           ...
                           pressure_recon::Bool=false, stage_bgk::Bool=false, Kn::Real=Inf, Pr::Real=1.0, omega::Real=0.5, stage_bgk_exact::Bool=false,
