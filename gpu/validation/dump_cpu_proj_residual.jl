# dump_cpu_proj_residual.jl — CPU residual_ho_3d!(order=2, use_proj_recon=true) reference
# for the GPU proj_first_order validation. MAIN env. Sets HO_VACUUM_FLOOR[]=0.001 to match
# the GPU vacf. Reads r3d.meta / r3d_M.f64 from $RIEMANN35_DATA, writes r3d_Rproj.f64.
using Riemann35
...
R=zeros(n+2g,n+2g,n,35); residual_ho_3d!(R,Mc,n,n,n,g,dx,dx,dx,Ma; order=2, use_proj_recon=true)
write(joinpath(DATA,"r3d_Rproj.f64"), reinterpret(UInt8, vec(Float64.(permutedims(R[g+1:g+n,g+1:g+n,:,:],(4,1,2,3))))))
println("wrote r3d_Rproj.f64 (CPU order=2 use_proj_recon, floor=0.001)")
