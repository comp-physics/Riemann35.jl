...
function run(USE, Np, halo, Ma, nsteps, dt)
    ...
    if USE; reset_anchor_stats!() else; ENV["HYQMOM_PROJ_COUNT"]="1"; reset_proj_counter!() end
    worst_nonfinite=0
    for s in 1:nsteps
    ...
    nfin=0;nrho=0;nun=0;mass1=0.0;en1=0.0
    for k in 1:nz,j in 1:ny,i in 1:nx
        c=@view M[i+halo,j+halo,k,:]
        for q in 1:35; isfinite(c[q])||(nfin+=1); end
        (c[1]>0.0)||(nrho+=1); (realizability_margin(c)<0.0)&&(nun+=1); mass1+=c[1]; en1+=c[3]+c[10]+c[20]
    end
    tag = USE ? "ON " : "OFF"
    @printf("%s(Ma=%g,%dstep,s3max=%g) nonfinite=%d rho<=0=%d unreal=%d massdrift=%.3e endrift=%.3e",
            tag,Ma,nsteps,s3max,nfin,nrho,nun, abs(mass1-mass0)/mass0, abs(en1-en0)/max(abs(en0),1e-300))
    if USE
        st=anchor_stats()
        @printf(" meantheta=%.4f theta_lt1=%d theta_min=%.4f would-regularize=%d fallback=%d\n",
                st.mean_theta, st.theta_lt1, st.theta_min, st.would_project, st.fallback)
    else
