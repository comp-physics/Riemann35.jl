# dump_cpu_hiorder3_residual_anchor.jl — CPU order-3 residual reference WITH the F3
# kinetic-FVS anchor (use_kfvs_anchor=true), for the GPU F3 parity gate. Same box/IC as
# dump_cpu_hiorder3_residual.jl; writes _anchor-suffixed R0/RN so the parity script can
# compare GPU F3 vs CPU F3. Run in the MAIN package env.
using Riemann35
...
residual_ho_3d_order3!(R, Mcpu, n, n, n, g, dx, dx, dx, Ma, 0.0; s3max=s3max, use_kfvs_anchor=true)
write(joinpath(DATA, "r3d_ho3_R0_anchor.f64"),
      reinterpret(UInt8, vec(Float64.(permutedims(R[g+1:g+n, g+1:g+n, :, :], (4,1,2,3))))))

R = zeros(n+2g, n+2g, n, 35)
residual_ho_3d_order3!(R, Mcpu, n, n, n, g, dx, dx, dx, Ma, dtN; s3max=s3max, use_kfvs_anchor=true)
write(joinpath(DATA, "r3d_ho3_RN_anchor.f64"),
      reinterpret(UInt8, vec(Float64.(permutedims(R[g+1:g+n, g+1:g+n, :, :], (4,1,2,3))))))

write(joinpath(DATA, "r3d_ho3_M.f64"), reinterpret(UInt8, vec(Float64.(U))))
write(joinpath(DATA, "r3d_ho3.meta"), "$n\n$dx\n$Ma\n$g\n$dtN\n$s3max\n")
println("wrote r3d_ho3_{R0,RN}_anchor.f64 + M + meta (CPU order-3 F3 anchor, n=$n g=$g Ma=$Ma)")
