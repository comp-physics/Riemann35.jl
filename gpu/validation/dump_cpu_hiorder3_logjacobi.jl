# dump_cpu_hiorder3_logjacobi.jl — CPU order-3 reference WITH use_logjacobi_recon=true,
# for the GPU log-Jacobi parity gate. Identical IC/halo to dump_cpu_hiorder3_residual.jl;
# only the flag differs. Writes r3d_ho3_lj_{R0,RN}.f64 (M/meta shared with the raw dump).
using Riemann35
...
residual_ho_3d_order3!(R, Mcpu, n, n, n, g, dx, dx, dx, Ma, 0.0; s3max=s3max, use_logjacobi_recon=true)
write(joinpath(DATA, "r3d_ho3_lj_R0.f64"),
      reinterpret(UInt8, vec(Float64.(permutedims(R[g+1:g+n, g+1:g+n, :, :], (4,1,2,3))))))
R = zeros(n+2g, n+2g, n, 35)
residual_ho_3d_order3!(R, Mcpu, n, n, n, g, dx, dx, dx, Ma, dtN; s3max=s3max, use_logjacobi_recon=true)
write(joinpath(DATA, "r3d_ho3_lj_RN.f64"),
      reinterpret(UInt8, vec(Float64.(permutedims(R[g+1:g+n, g+1:g+n, :, :], (4,1,2,3))))))
...
println("wrote r3d_ho3_lj_{R0,RN}.f64 (CPU order-3 log-Jacobi, n=$n g=$g Ma=$Ma)")
